...
extension String {
    public func bytes(_ using: Encoding = .utf8) -> Int {
        self.data(using: using)?.count ?? 0
    }
    
    // TO DECODABLE STRUCT
    public func toModel<T>(_ model: T.Type) -> T? where T : Decodable {
        guard let data = self.data(using: String.Encoding.utf8) else { return nil }
        return try? JSONDecoder().decode(model, from: data)
    }
    
    public func toModel<T>(_ model: T.Type) throws -> T where T : Decodable {
    ...
    public func getPointer<T>(_ handler: (_ pointer: UnsafePointer<T>, _ len: Int) throws -> Void) rethrows {
        var string = self
